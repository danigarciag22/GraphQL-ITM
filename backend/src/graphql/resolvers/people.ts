...
const peopleResolver: IResolvers = {
  Query: {
    ...
    getPersonByName: (parent, { name }) => {
      return peopleDataSource.filter((peopleDataSource) =>
        peopleDataSource.name.toLowerCase().includes(name.toLowerCase())
      );
    },
  },
  Mutation: {
    createPersonInMongo: async (root: void, args: any, context: Db) => {
      try {
        const person = await context
          .collection("people")
          .insertOne(args.person);
    ...
    updatePerson: (parent, { _id, input }) => {
      const personIndex = peopleDataSource.findIndex(
